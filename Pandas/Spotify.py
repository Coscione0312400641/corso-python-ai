...
print(X_final.shape)
similar_track=10
model=NearestNeighbors(n_neighbors=similar_track+1,metric="euclidean")
model.fit(X_num_scaled)
print(len(df))
print(df[feature_num].describe())
...
def reccomend_by_track_id(
    ...
    distances,indices=model.kneighbors(seed_vec,k)
    recs=df.iloc[indices[0]].copy()
    recs[recs["track_id"]!=track_id]
    if same_genre:
    ...
    return recs[cols]
test_id="1UKp5X9cPp9vMnTb39nnvg"
print("Traccia seed: \n")
